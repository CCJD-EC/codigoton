	...
	public static void writeOutput(ArrayList<Table> content) {
		...
		try {
			...
			for (int i = 0; i < content.size(); i++) {
				...
				outP.println("<" + content.get(i).getName() + ">");
				
				if (content.get(i).isEnableTable()) {
					for (int x = 0; x < content.get(i).getClients().size(); x++) {
						if (x != 0)
							outP.print(",");
						
						outP.print(content.get(i).getClients().get(x).getCode());
					}
				} else {
					outP.print("CANCELADA");
				}
			}
